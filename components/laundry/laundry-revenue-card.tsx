...
import { getLaundryRevenue } from '@/lib/laundry-actions'
...
export function LaundryRevenueCard({ currency }: { currency: string }) {
  const [r, setR] = useState<R | null>(null)
  const load = useCallback(async () => {
    try { setR(await getLaundryRevenue()) } catch { /* ignore */ }
  }, [])
  useEffect(() => { load() }, [load])
  if (!r) return null
  return (
